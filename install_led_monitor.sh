...
echo "=========================================="
echo "1) Universal Mode (Global Ping Test)"
echo "   -> Best for Single WAN, Access Points, Repeaters, & USB Tethering."
echo "   -> Ignores all interface rules; simply checks if the internet is reachable."
echo ""
echo "2) Strict Dual-WAN Mode"
echo "   -> Assign specific LED colors to specific WAN interfaces."
echo "   -> Supports mwan3 integration for policy routing."
echo ""
echo "3) Cancel / Abort"
...
# --- Cache mwan3 status to save massive CPU time (STRICT MODE ONLY) ---
MWAN_STATUS=""
if [ "$MONITOR_MODE" = "strict_dual" ] && command -v mwan3 >/dev/null 2>&1; then
...
if [ "$PORT_WARNING" -eq 1 ]; then
    ...
    if [ "$MONITOR_MODE" = "strict_dual" ]; then
        ...
        log "Evaluating Universal Rules (Global Ping)..."
